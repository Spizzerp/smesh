...
/// BIP-39 Mnemonic implementation for wallet seed generation
/// Reference: https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki
public final class BIP39: Sendable {
    ...
    public enum Error: Swift.Error, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .invalidWordCount:
                return "Mnemonic must have 12, 15, 18, 21, or 24 words"
            case .wordNotInList(let word):
                return "Word '\(word)' is not in BIP-39 wordlist"
            case .checksumMismatch:
    ...
    /// - Parameter strength: Entropy bits (128 = 12 words, 256 = 24 words)
    ...
    /// - Parameter entropy: Random entropy bytes (16, 20, 24, 28, or 32 bytes)
    /// - Returns: Array of mnemonic words
    public static func entropyToMnemonic(_ entropy: [UInt8]) throws -> [String] {
        let strength = entropy.count * 8
        guard [128, 160, 192, 224, 256].contains(strength) else {
            throw Error.invalidStrength
        }

        // 2. Calculate checksum (first CS bits of SHA256)
        let hash = SHA256.hash(data: Data(entropy))
        let checksumBits = strength / 32

        // 3. Convert entropy to bits
        var bits = entropy.flatMap { byte -> [UInt8] in
            (0..<8).reversed().map { UInt8((byte >> $0) & 1) }
        }

        // 4. Append checksum bits
        let hashBytes = Array(hash)
        let checksumBitsArray: [UInt8] = hashBytes.flatMap { byte -> [UInt8] in
            (0..<8).reversed().map { UInt8((byte >> $0) & 1) }
        }
        bits.append(contentsOf: checksumBitsArray.prefix(checksumBits))

        // 5. Convert to 11-bit word indices
        var words: [String] = []
    ...
    /// - Parameter phrase: Array of mnemonic words
    /// - Returns: true if valid
    public static func isValid(phrase: [String]) -> Bool {
        do {
            try validate(phrase: phrase)
            return true
        } catch {
            return false
        }
    ...
    /// Validate a mnemonic phrase (throwing version)
    /// - Parameter phrase: Array of mnemonic words
    /// - Throws: Error if invalid
    public static func validate(phrase: [String]) throws {
        // Check word count
        guard [12, 15, 18, 21, 24].contains(phrase.count) else {
            throw Error.invalidWordCount
        }

        // Check all words are in list and convert to indices
        var bits: [UInt8] = []
        for word in phrase {
            guard let index = wordlist.firstIndex(of: word.lowercased()) else {
                throw Error.wordNotInList(word)
            }
        ...
        for byte in hashBytes {
            for i in (0..<8).reversed() {
                expectedChecksumBits.append(UInt8((byte >> i) & 1))
                if expectedChecksumBits.count >= checksumLength {
                    break
                }
            }
            if expectedChecksumBits.count >= checksumLength {
        ...
        // Verify checksum
        guard checksumBits == Array(expectedChecksumBits.prefix(checksumLength)) else {
    ...
    /// Derive seed from mnemonic (PBKDF2-SHA512, 2048 rounds)
    /// - Parameters:
    ///   - phrase: Array of mnemonic words
    ///   - passphrase: Optional passphrase (default empty)
    /// - Returns: 64-byte seed
    public static func mnemonicToSeed(phrase: [String], passphrase: String = "") -> Data {
        let mnemonic = phrase.joined(separator: " ").decomposedStringWithCompatibilityMapping
    ...
    /// Derive seed from mnemonic string
    /// - Parameters:
    ///   - mnemonicString: Space-separated mnemonic words
    ///   - passphrase: Optional passphrase
    /// - Returns: 64-byte seed
    public static func mnemonicToSeed(mnemonicString: String, passphrase: String = "") -> Data {
        let phrase = mnemonicString.split(separator: " ").map(String.init)
        return mnemonicToSeed(phrase: phrase, passphrase: passphrase)
    }
...
extension BIP39 {
    ...
    public static func generate12WordMnemonic() throws -> [String] {
        try generateMnemonic(strength: 128)
    }

    /// Generate a 24-word mnemonic (256-bit entropy)
    public static func generate24WordMnemonic() throws -> [String] {
        try generateMnemonic(strength: 256)
    }

    /// Parse a mnemonic string into words
    public static func parse(_ mnemonicString: String) -> [String] {
        mnemonicString
            .lowercased()
